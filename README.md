...
Разгружает длинные переписки в ChatGPT: удаляет старые сообщения из DOM, оставляя последние 4 — Chrome перестаёт лагать на огромных тредах.

## Фичи

- Очищает DOM сообщений **только в рамках диалога** (внутри `main`)
- Оставляет **последние 4** сообщения
- Триггеры:
  - кнопка в UI ChatGPT (**появляется только если сообщений > 4**)
  - popup расширения
...
- **Popup**: иконка расширения → `Очистить DOM (оставить 4)`
- **Контекстное меню**: ПКМ по странице → `Очистить DOM узлы в чате (оставить 4)`
